...
Deno.test("resolveRos2InstallPlan normalises container config", () => {
  ...
  assertEquals(plan.env, {
    RMW_IMPLEMENTATION: "rmw_cyclonedds_cpp",
    PSYCHED_WORKSPACE_DIR: "/workspace",
    PSYCHED_REPO_ROOT: repoRoot(),
    PSYCHED_ROS_MODE: "container",
    ROS_DOMAIN_ID: getRosDomainId(),
    ROS_LOCALHOST_ONLY: "0",
...
Deno.test("renderRosContainerProfile emits helper alias", () => {
  ...
  const expected = [
    ...
    "export PSYCHED_ROS2_HELPER='/usr/local/bin/ros2-container'",
    `export PSYCHED_ROS_CONTAINER_WORKSPACE='/work'`,
    `export PSYCHED_ROS_CONTAINER_REPO='${repoRoot()}'`,
