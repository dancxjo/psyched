...
import { repoRoot, workspaceRoot } from "../paths.ts";
...
export function renderRosProfile(distro: string): string {
  ...
  return [
    ...
    "export PSYCHED_ROS_MODE=native",
    "unset PSYCHED_ROS2_HELPER",
...
export function resolveRos2InstallPlan(
  ...
  const repoSource = readString(
    containerRaw?.repo_source ?? raw?.repo_source,
    "ros2.container.repo_source",
  ) ?? repoRoot();
  const repoTarget = readString(
    containerRaw?.repo_target ?? raw?.repo_target,
    "ros2.container.repo_target",
  ) ?? repoSource;
  ...
  if (!envRecord.PSYCHED_WORKSPACE_DIR) {
    envRecord.PSYCHED_WORKSPACE_DIR = workspaceTarget;
  }
  if (!envRecord.PSYCHED_REPO_ROOT) {
    envRecord.PSYCHED_REPO_ROOT = repoTarget;
  }
  if (!envRecord.PSYCHED_ROS_MODE) {
    envRecord.PSYCHED_ROS_MODE = "container";
  }
  if (!envRecord.ROS_DOMAIN_ID) {
    envRecord.ROS_DOMAIN_ID = getRosDomainId();
  }
  if (!envRecord.RMW_IMPLEMENTATION) {
    envRecord.RMW_IMPLEMENTATION = "rmw_cyclonedds_cpp";
  }
  if (!envRecord.ROS_LOCALHOST_ONLY) {
    envRecord.ROS_LOCALHOST_ONLY = "0";
  }
...
export function renderRosContainerProfile(
  ...
  const workspace = shellQuote(options.workspaceTarget);
  const repo = shellQuote(options.repoTarget);
  return [
    ...
    `export ROS_DOMAIN_ID=${domainId}`,
    "export ROS_LOCALHOST_ONLY=0",
    "export PSYCHED_ROS_MODE=container",
    `export PSYCHED_ROS2_HELPER=${helper}`,
    `export PSYCHED_ROS_CONTAINER_WORKSPACE=${workspace}`,
    `export PSYCHED_ROS_CONTAINER_REPO=${repo}`,
...
function buildContainerHelperScript(
  ...
  const script: string[] = [
    ...
    `readonly repo_source=${shellQuote(repoSource)}`,
    `readonly repo_target=${shellQuote(repoTarget)}`,
    `readonly repo_mount=${shellQuote(`${repoSource}:${repoTarget}:rw`)}`,
...
async function installRos2Container(
  ...
  const repoSource = plan.repoSource ?? repoRoot();
  const repoTarget = plan.repoTarget ?? repoSource;
  const repoMountKey = `${repoSource}:${repoTarget}`;
  const additionalVolumes = plan.volumes.filter((volume) =>
    volume !== workspaceMountKey &&
    volume !== `${workspaceMountKey}:rw` &&
    volume !== repoMountKey &&
    volume !== `${repoMountKey}:rw`
  );
