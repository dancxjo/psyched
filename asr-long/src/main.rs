...
use anyhow::Context;
use pipeline::{LongPipeline, LongPipelineConfig};
...
async fn main() -> anyhow::Result<()> {
    ...
    let pipeline = Arc::new(LongPipeline::new(LongPipelineConfig::default()));
    let app = server::router(pipeline);
    info!(%listen, "starting asr-long websocket server");
