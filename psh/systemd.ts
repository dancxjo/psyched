...
function buildUnitContent(
  ...
  const launchScript = spec.launch ?? `modules/${moduleName}/launch.sh`;
  const shutdownScript = spec.shutdown ?? `modules/${moduleName}/shutdown.sh`;
  const launchPath = join(repoDir, launchScript);
  const shutdownPath = join(repoDir, shutdownScript);
...
ExecStop=${entrypoint} bash -c \"\\\"${shutdownPath}\\\"\"
Restart=${restart}
RestartSec=${restartSec}
User=${user}
KillMode=${killMode}
TimeoutStopSec=${timeoutStop}
