...
- **ROS workspace mount:** The ROS 2 container helper binds the repository's `work/` directory to `/work` inside the container. Keep overlays and build outputs inside `work/` so they are visible during containerised builds.
