...
# Gracefully stop the ear module's ros2 launch
...
kill_with_pattern() {
  ...
  echo "[ear/shutdown] Stopping ${#pids[@]} process(es) for: $pattern (SIGTERM)"
  for pid in "${pids[@]}"; do
    if [ "$pid" -ne $$ ]; then
  ...
  echo "[ear/shutdown] Forcing stop (SIGKILL) for remaining processes"
