...
start_background() {
  ...
  rm -f "${pid_file}"
  echo "[cockpit/launch] Starting ${key}: $*" >&2
  "$@" >>"${log}" 2>&1 &
  local child=$!
  echo "${child}" > "${pid_file}"
...
cleanup() {
  ...
  if [[ -n "${COCKPIT_PID:-}" ]] && kill -0 "${COCKPIT_PID}" 2>/dev/null; then
    echo "[cockpit/launch] Stopping cockpit (PID ${COCKPIT_PID})" >&2
    kill "${COCKPIT_PID}" 2>/dev/null || true
  fi

  for pid in "${BACKGROUND_PIDS[@]}"; do
    if kill -0 "${pid}" 2>/dev/null; then
      echo "[cockpit/launch] Stopping helper process (PID ${pid})" >&2
...
CONFIG_CANDIDATE="${COCKPIT_HOST_CONFIG:-}"
...
FRONTEND_ROOT="${COCKPIT_FRONTEND_ROOT:-${REPO_DIR}/modules/cockpit/packages/cockpit/cockpit/frontend}"
export COCKPIT_FRONTEND_ROOT="${FRONTEND_ROOT}"
if [[ ! -d "${FRONTEND_ROOT}" ]]; then
  echo "ERROR: Could not find cockpit frontend assets." >&2
  exit 1
fi

if [[ -z "${COCKPIT_MODULES_ROOT:-}" ]]; then
  export COCKPIT_MODULES_ROOT="${REPO_DIR}/modules"
fi
...
if [[ -n "${COCKPIT_LOG_LEVEL:-}" ]]; then
  ARGS+=("--log-level" "${COCKPIT_LOG_LEVEL}")
fi

if [[ -f "${REPO_DIR}/work/install/cockpit/share/cockpit/local_setup.bash" ]]; then
  set +u
  # shellcheck disable=SC1091
  source "${REPO_DIR}/work/install/cockpit/share/cockpit/local_setup.bash"
  set -u
fi

echo "[cockpit/launch] Starting cockpit: ros2 run cockpit cockpit_cockpit ${ARGS[*]}" >&2
ros2 run cockpit cockpit_cockpit "${ARGS[@]}" &
COCKPIT_PID=$!
