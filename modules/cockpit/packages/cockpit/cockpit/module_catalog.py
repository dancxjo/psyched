"""Module metadata loader for the simplified cockpit backend."""
...
class ModuleInfo:
    """Metadata describing a module's cockpit-facing assets."""
    ...
    def to_dict(self) -> Dict[str, object]:
        payload: Dict[str, object] = {
            ...
            "slug": self.slug,
            "has_cockpit": self.cockpit_assets,
        }
...
class ModuleCatalog:
    ...
    def _load(self) -> Dict[str, ModuleInfo]:
        ...
        for entry in sorted(self.modules_root.iterdir()):
            ...
            cockpit_dir = entry / "cockpit"

            if manifest.exists():
                info = self._parse_manifest(entry.name, manifest, cockpit_dir)
            else:
                info = ModuleInfo(
                    name=entry.name,
                    display_name=entry.name.replace("_", " ").title(),
                    description="",
                    cockpit_assets=cockpit_dir.exists(),
                )
    ...
    def _parse_manifest(self, module_name: str, manifest: Path, cockpit_dir: Path) -> ModuleInfo:
        ...
        cockpit_cfg = self._coerce_mapping(manifest_data.get("cockpit"))

        display_name = str(
            cockpit_cfg.get("display_name")
            or manifest_data.get("display_name")
            or name.replace("_", " ").title(),
        )
        description = str(
            cockpit_cfg.get("description")
