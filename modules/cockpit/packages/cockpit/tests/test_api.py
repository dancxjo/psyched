"""Regression tests for the streamlined cockpit HTTP API."""
...
[host]
name = "api-test"
modules = ["imu", "cockpit", "foot", "viscera"]

[config.mod.imu.launch]
enabled = true

[config.mod.cockpit.launch]
enabled = true
...
def test_modules_endpoint_reports_active_dashboards(
    ...
    monkeypatch.setattr("cockpit.server._module_systemd_status", fake_status)
...
async def _exercise_modules_endpoint(config_file: Path, tmp_path: Path) -> None:
    ...
    assert module_names == {"imu", "cockpit", "foot", "viscera"}

    foot_entry = next(module for module in modules if module["name"] == "foot")
    assert foot_entry["has_cockpit"] is True
    assert foot_entry["dashboard_url"].endswith("/modules/foot/")

    imu_entry = next(module for module in modules if module["name"] == "imu")
    assert imu_entry["has_cockpit"] is True

    cockpit_entry = next(module for module in modules if module["name"] == "cockpit")
    assert cockpit_entry["has_cockpit"] is True
    assert cockpit_entry["dashboard_url"].endswith("/modules/cockpit/")

    viscera_entry = next(module for module in modules if module["name"] == "viscera")
    assert viscera_entry["has_cockpit"] is True
...
async def _exercise_module_config_endpoint(config_file: Path, tmp_path: Path) -> None:
    ...
    async with _run_app(app) as client:
        ...
        assert set(modules) == {"imu", "cockpit", "foot", "viscera"}
