...
import { createTopicSocket, callRosService } from '/js/pilot.js';
...
const TELEMETRY_TOPICS = [
  ...
  { topic: 'joint_states', type: 'sensor_msgs/msg/JointState', handler: 'updateJointStates' },
  { topic: 'mode', type: 'create_msgs/msg/Mode', handler: 'updateMode' },
  { topic: 'odom', type: 'nav_msgs/msg/Odometry', handler: 'updateOdometry' },
  { topic: 'main_brush_motor', type: 'std_msgs/msg/Float32', handler: 'updateMainBrushMotor' },
  { topic: 'side_brush_motor', type: 'std_msgs/msg/Float32', handler: 'updateSideBrushMotor' },
  { topic: 'vacuum_motor', type: 'std_msgs/msg/Float32', handler: 'updateVacuumMotor' },
  { topic: 'diagnostics', type: 'diagnostic_msgs/msg/DiagnosticArray', handler: 'updateDiagnostics' },
  { topic: 'parameter_events', type: 'rcl_interfaces/msg/ParameterEvent', handler: 'updateParameterEvent' },
  { topic: 'tf', type: 'tf2_msgs/msg/TFMessage', handler: 'updateTf' },
  { topic: 'tf_static', type: 'tf2_msgs/msg/TFMessage', handler: 'updateTfStatic' },
];
...
class FootDashboard extends LitElement {
  ...
  updateBattery(state) {
    ...
    const percent = ratio != null ? clampPercent(ratio * 100) : percentFromCharge(charge, capacity);
  ...
  updateIrOmni(message) {
    ...
    this.telemetry = {
      ...this.telemetry,
      irOmni: Number.isFinite(code) && code > 0 ? `IR code ${code}` : 'No IR signal',
    };
  }
  ...
  updateTf(message) {
    const count = Array.isArray(message?.transforms) ? message.transforms.length : 0;
    this.telemetry = {
      ...this.telemetry,
      tf: {
  ...
  pushButtonEvent(label, topic) {
    const entry = {
      id: crypto.randomUUID ? crypto.randomUUID() : `btn-${Date.now()}-${Math.random()}`,
      label,
      topic,
      time: new Date().toLocaleTimeString(),
    };
  ...
  sendAscii(event) {
    ...
    try {
      ...
      const socket = this.ensurePublisher('set_ascii', 'std_msgs/msg/UInt8MultiArray');
  ...
  sendPowerLed(event) {
    event.preventDefault();
    try {
      const payload = buildPowerLedPayload(this.displayState.powerColor, this.displayState.powerIntensity);
      const socket = this.ensurePublisher('power_led', 'std_msgs/msg/UInt8MultiArray');
      socket.send(JSON.stringify(payload));
  ...
  async submitParameters(event) {
    ...
    if (!node || !node.trim()) {
      this.parameterStatus = 'Target node path is required (example: /create_driver).';
      return;
    }
    ...
    try {
      ...
      this.parameterStatus = error instanceof Error ? error.message : String(error);
    } finally {
      this.parameterBusy = false;
    }
  }
  ...
  async submitSongDefinition(event) {
    ...
    if (!notes.length) {
      this.songStatus = 'Provide at least one "note,duration" pair to define a song.';
      return;
    }
    ...
    try {
      const socket = this.ensurePublisher('define_song', 'create_msgs/msg/DefineSong');
      socket.send(
        JSON.stringify({
          id,
          notes: notes.map((entry) => ({ note: entry.note, duration: entry.duration })),
        }),
      );
    ...
    return html`
        ...
        <article class="surface-card">
          ...
          <div class="surface-grid surface-grid--dense surface-grid--narrow">
            ...
            <div class="surface-metric">
              ...
              <span class="surface-metric__value">${this.battery.capacityDisplay}</span>
            </div>
            <div class="surface-metric">
              <span class="surface-metric__label">Charge ratio</span>
              <span class="surface-metric__value">${this.battery.ratioDisplay}</span>
            </div>
            <div class="surface-metric">
          ...
          return html`
                <label>
                  ...
                  <input
                    type="checkbox"
                    .checked=${checked}
                    @change=${(event) => this.toggleLed(name, event.target.checked)}
          ...
          <form @submit=${(event) => this.sendAscii(event)}>
            <label>
              ...
              <input
                ...
                @input=${(event) => this.updateDisplayField('ascii', event.target.value.toUpperCase())}
              />
            </label>
          ...
          <form @submit=${(event) => this.sendPowerLed(event)}>
            <label>
              ...
              <input
                ...
                @input=${(event) => this.updateDisplayField('powerColor', Number(event.target.value))}
              />
            </label>
            <label>
              LED intensity
              <input
                ...
                @input=${(event) => this.updateDisplayField('powerIntensity', Number(event.target.value))}
              />
            </label>
            <div class="surface-actions">
        ...
        <article class="surface-card">
          ...
          <form @submit=${(event) => this.submitSongDefinition(event)}>
            <div class="surface-grid surface-grid--dense">
              <label>
                ...
                <input
                  ...
                  @input=${(event) => this.updateSongField('id', event.target.value)}
                  required
                />
              </label>
              <label>
                ...
                <textarea
                  ...
                  @input=${(event) => this.updateSongField('notes', event.target.value)}
                  placeholder="60,0.5\n64,0.25"
                ></textarea>
              </label>
            </div>
            <div class="surface-actions surface-actions--grid">
              ...
              <button class="surface-action" type="button" @click=${(event) => this.playSong(event)}>Play song</button>
            </div>
          </form>
      ...
      <div class="surface-grid surface-grid--wide" style="margin-top: 1rem;">
        <article class="surface-card">
          ...
          <div class="telemetry-grid">
            ...
            <div class="surface-metric">
              ...
              <span class="surface-metric__value">${this.telemetry.irOmni}</span>
            </div>
            <div class="surface-metric">
            ...
            <div class="surface-metric">
              ...
              <span class="surface-metric__value">${this.telemetry.tf.count} frames · ${this.telemetry.tf.last}</span>
            </div>
            <div class="surface-metric">
              <span class="surface-metric__label">TF static</span>
              <span class="surface-metric__value">${this.telemetry.tfStatic.count} frames · ${this.telemetry.tfStatic.last}</span>
            </div>
            <div class="surface-metric">
              <span class="surface-metric__label">Main brush</span>
              <span class="surface-metric__value">${this.telemetry.mainBrush}</span>
            </div>
            <div class="surface-metric">
              <span class="surface-metric__label">Side brush</span>
              <span class="surface-metric__value">${this.telemetry.sideBrush}</span>
            </div>
            <div class="surface-metric">
        ...
        <article class="surface-card">
          ...
          <pre class="surface-panel surface-mono" style="max-height: 220px; overflow-y: auto;">${this.telemetry.diagnostics}</pre>
        </article>
