...
class DepthToScan:
    ...
    def project_depth_to_scan(self, depth_img):
        """
        Projects depth image to a 1D laser scan by finding the nearest obstacle for each column
        within the robot's height range (min_y to max_y).
        Returns:
            scan_ranges: list of float, nearest range per column
            line_mask: np.ndarray, mask of selected pixels (for overlay)
        """
        h, w = depth_img.shape
        scan_ranges = []
