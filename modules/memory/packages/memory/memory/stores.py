...
class GraphStoreProtocol(Protocol):
    ...
    def fetch_memories(
        ...
        """Return metadata describing multiple stored memory nodes in a single batch."""
...
class Neo4jGraphStore(GraphStoreProtocol):
    ...
    def _read_memories(tx, memory_ids: list[str]) -> Mapping[str, Mapping[str, Any]]:
        # ⚡ Bolt Optimization: Use UNWIND to batch fetch multiple memory nodes
        # in a single database round-trip, resolving the N+1 query problem.
        result = tx.run(
            """
