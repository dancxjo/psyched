...
class QdrantVectorStore(VectorStoreProtocol):
    ...
    def __init__(self, client: Any) -> None:  # pragma: no cover - simple assignment
        self._client = client
...
class Neo4jGraphStore(GraphStoreProtocol):
    ...
    def fetch_memories(
        ...
        if not memory_ids:
            return {}
        with self._driver.session() as session:
            return session.execute_read(self._read_memories, list(memory_ids))
    ...
    def _read_memories(tx, memory_ids: List[str]) -> Mapping[str, Mapping[str, Any]]:
        result = tx.run(
            ...
            MATCH (memory:Memory {memory_id: mem_id})
            OPTIONAL MATCH (memory)-[:IDENTIFIES]->(identity:Identity)
            RETURN memory, collect(identity) AS identities
            """,
