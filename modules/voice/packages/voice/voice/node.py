...
from std_msgs.msg import UInt32
            ...
            try:
                ...
                # Engine defaults
                ENGINE = os.getenv("VOICE_ENGINE", "espeak-ng")  # espeak-ng only
                # Default espeak-ng voice (use MBROLA voice like 'mbrola-en1' if installed)
                ESPEAK_VOICE = os.getenv("ESPEAK_VOICE", "mbrola-en1")


                class VoiceNode(Node):
                    """Speak queued messages via espeak-ng TTS (with mbrola-en1).

                    Args:
                        espeak_voice: espeak-ng/MBROLA voice name (e.g., "mbrola-en1").

                    Examples:
                        >>> VoiceNode(espeak_voice="mbrola-en1")  # doctest: +SKIP
                    """


                    def __init__(self, espeak_voice: str = ESPEAK_VOICE) -> None:
                        super().__init__('voice')
                        self.declare_parameter('topic', '/voice')
                        self.declare_parameter('engine', os.environ.get('VOICE_ENGINE', 'espeak-ng'))  # 'espeak-ng' or 'pyttsx3'
                        self.declare_parameter('espeak_voice', os.environ.get('ESPEAK_VOICE', 'mbrola-en1'))
                        ...
                        self.declare_parameter('pyttsx3_voice', os.environ.get('PYTTSX3_VOICE', 'english'))
                        self.declare_parameter('pyttsx3_rate', 170)
                        self.declare_parameter('pyttsx3_volume', 1.0)
                        ...
                        def _run_worker(self):
                            ...
                            import shutil
                            engine = self.get_parameter('engine').get_parameter_value().string_value.strip().lower()
                            ...
                            pyttsx3_voice = self.get_parameter('pyttsx3_voice').get_parameter_value().string_value
                            pyttsx3_rate = int(self.get_parameter('pyttsx3_rate').get_parameter_value().integer_value or 170)
                            pyttsx3_volume = float(self.get_parameter('pyttsx3_volume').get_parameter_value().double_value or 1.0)

                            # Setup pyttsx3 engine if needed
                            pyttsx3_engine = None
                            if engine == 'pyttsx3':
                                try:
                                    import pyttsx3
                                    pyttsx3_engine = pyttsx3.init()
                                    pyttsx3_engine.setProperty('voice', pyttsx3_voice)
                                    pyttsx3_engine.setProperty('rate', pyttsx3_rate)
                                    pyttsx3_engine.setProperty('volume', pyttsx3_volume)
                                except Exception as e:
                                    self.get_logger().error(f'pyttsx3 initialization failed: {e}')
                                    pyttsx3_engine = None
                            ...
                            while not self._stop_event.is_set():
                                ...
                                success = False
                                if engine == 'pyttsx3' and pyttsx3_engine:
                                    try:
