...
- Operators should use the dedicated HTTPS harness (`ear_browser_bridge`) to stream audio/video when mic or camera access is required.
...
4. To publish browser media, start the `ear_browser_bridge` HTTPS server and open `/ear` or `/eye` from that host; the pilot cockpit no longer exposes browser hooks directly.
