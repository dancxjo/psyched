...
export class CockpitClient {
    ...
    subscribe<T>(topic: string, callback: TopicCallback<T>, options: SubscribeOptions = {}): () => void {
        if (!isBrowser) {
            return () => undefined;
        }
    ...
    private async handleInbound(raw: unknown): Promise<void> {
        ...
        try {
            payload = typeof payloadRaw === "string" ? JSON.parse(payloadRaw) : payloadRaw as WsOutboundMessage;
        } catch (error) {
        ...
        switch (payload.op) {
            ...
            case "err": {
                console.error("Cockpit reported error", payload.reason ?? "unknown error");
                break;
            }
            case "ok":
            default:
